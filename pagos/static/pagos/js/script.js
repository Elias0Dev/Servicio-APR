    ...
    navItems.forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            switch(section) {
                ...
                case 'pagos':
                    // ** MODIFICACIÓN CRUCIAL: Carga la vista de Pagos desde Django (AJAX/Fetch) **
                    fetch('/pagos/content/') 
                        .then(response => {
                            if (!response.ok) {
                                throw new Error('Error al cargar la vista de Pagos. Código: ' + response.status);
                            }
                            return response.text();
                        })
                        .then(html => {
                            // Inyecta el HTML renderizado por Django (pagos.html)
                            contentArea.innerHTML = html;
                        })
                        .catch(error => {
                            console.error('Fallo en la petición de Pagos:', error);
                            contentArea.innerHTML = `<div class="card"><h1>Error</h1><p>No se pudo cargar el módulo de Pagos. Revise la URL '/pagos/content/' y la vista en Django.</p></div>`;
                        });
                    break;
